...
reproduce the stated value of `alpha`, which should be `alpha - delta = -3.5`.
...
get_slope1 <- function (xmin = 1, alpha = 3, delta = 0.5, len = 1e4)
{
    ...
    res <- c (get_plmod (alpha),
              get_plmod (c (alpha0, alpha)))
    names (res) <- c ("O2", "O3")
    return (res)
}
delta <- 1:10 / 10
...
These observed lines do not really reflect the expected ("theoretical") values,
although the clearly follow the same trajectories.
```{r fit-hetero-conv, eval = FALSE, echo = FALSE}
pareto_optimise (m, x0 = 1, n = 0, quiet = FALSE)
```
...
# Fitting tails of simulated values

The previous procedures employed linear regressions, which are statistically
invalid, as explained by both Clauset and Gillepie. The procedure developed
here, and in the manuscript, employs the `fit_ks_model` function given above.
The following is a modification of the previous code to include that.

```{r}
ks_stat <- function (pars, x, yobs, k = 10)
{
    ymod <- pars [1] + x * pars [2]
    n <- 0
    while (abs (k - 1) > 1e-10)
    {
        i1 <- which.min (yobs - ymod)
...
yz <- fz (z)
#yz <- yz * min (y0) / min (yz [yz > 0])
optim (c (-1, -3), ks_stat, x = log10 (z), yobs = log10 (yz), k = 10,
       method = "BFGS")
```
